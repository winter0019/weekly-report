    ...
    <div className="min-h-screen bg-slate-50 flex flex-col pb-6">
      ...
      <div className="pt-14 px-4 sm:px-6 max-w-[1200px] mx-auto w-full">
        <header className="bg-[#004d40] text-white p-4 shadow-xl rounded-lg flex flex-wrap items-center justify-between no-print gap-4 mb-5 animate-official">
          ...
          <div className="flex items-center gap-3">
            <a href={activeFormUrl} target="_blank" rel="noopener noreferrer" className="bg-emerald-500/20 hover:bg-emerald-500/40 px-3 py-1 rounded text-[8px] font-bold uppercase tracking-wider border border-white/10 transition-colors flex items-center gap-1">
            ...
            {userRole === 'ZI' && (
              <button onClick={() => setIsSettingsOpen(true)} className="w-7 h-7 bg-white/10 hover:bg-white/20 rounded transition-all flex items-center justify-center border border-white/10">
                <SpreadsheetIcon />
              </button>
            )}
...
const CIMModule = ({ entries, db, lga, userRole, stationDispositions }: any) => {
  ...
  const handleSaveStationDisposition = async () => {
    ...
    try {
      ...
      window.alert("Saved Population Data.");
  ...
  const handleIssueQuery = async (cm: any) => {
    ...
    try {
      ...
      window.alert("Query Issued and PDF Generated.");
  ...
  const handleSubmitAudit = async (e: any) => {
    e.preventDefault();
    if (!formData.month) return alert("Enter month.");
    ...
    window.alert("Monthly Audit Published.");
  };
      ...
      <div className="w-full flex flex-col gap-5 animate-official">
        ...
        <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
           <div className="p-3 bg-slate-800 text-white flex justify-between items-center cursor-pointer hover:bg-black transition-all" onClick={() => setIsDispBreakdownOpen(!isDispBreakdownOpen)}>
              <div className="flex items-center gap-3">
                 <DashboardIcon />
                 <h3 className="text-[11px] font-black uppercase tracking-widest">Zonal Batch Distribution</h3>
              </div>
             ...
             <div className="p-4 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3 bg-slate-50 animate-official border-b">
                {Object.entries(aggregates.zonalBatches).map(([batch, counts]: [string, any]) => (
                  <div key={batch} className="p-3 bg-white rounded border border-slate-200 shadow-sm flex flex-col items-center">
                     ...
                     <p className="text-[8px] font-bold text-slate-400 uppercase mt-2">Total: {counts.males + counts.females}</p>
                  </div>
                ))}
             </div>
           )}
        ...
        <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
          <div className="bg-[#004d40] p-3 text-white flex justify-between items-center">
             ...
             <div className="flex gap-2">
                ...
                <button onClick={() => setIsLedgerOpen(true)} className="px-3 py-1 bg-white/10 hover:bg-white/20 rounded text-[9px] font-bold uppercase border border-white/20">Exceptions Registry</button>
             </div>
          </div>
                          ...
                          <tr key={idx} className="hover:bg-slate-50 transition-all">
                            ...
                            <td className="py-2 text-right">
                               <div className="flex items-center justify-end gap-1.5">
                                 <button onClick={() => handleIssueQuery(cm)} disabled={isGenerating} className="px-3 py-1 bg-[#004d40] text-white text-[8px] font-bold uppercase rounded hover:bg-black disabled:opacity-50 transition-all">Query</button>
        ...
        <div className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm">
          ...
          <form onSubmit={handleSubmitAudit} className="space-y-2">
            ...
            <div className="p-2 bg-emerald-50/40 rounded border border-emerald-100 space-y-1.5">
               <label className="text-[7px] font-bold uppercase text-emerald-800">Assign Cleared Population</label>
               ...
               <div className="grid grid-cols-2 gap-1.5">
                  <input type="number" placeholder="M CLEARED" className="p-1.5 bg-white rounded border border-slate-200 text-[9px] font-bold" value={newClearedBatch.males || ''} onChange={e => setNewClearedBatch({...newClearedBatch, males: parseInt(e.target.value) || 0})} />
                  <input type="number" placeholder="F CLEARED" className="p-1.5 bg-white rounded border border-slate-200 text-[9px] font-bold" value={newClearedBatch.females || ''} onChange={e => setNewClearedBatch({...newClearedBatch, females: parseInt(e.target.value) || 0})} />
               </div>
            ...
            <button className="w-full bg-[#004d40] text-white p-2 rounded font-bold uppercase text-[8px] shadow-sm">Submit to Zonal HQ</button>
          </form>
        </div>
      </div>
      ...
      <div className="flex-1 space-y-4">
        <div className="bg-slate-900 rounded-lg shadow-xl p-5 text-white flex flex-col md:flex-row justify-between items-center gap-6 animate-official relative overflow-hidden">
           ...
           <div className="z-10 text-center md:text-left">
              <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-400 mb-1">Station Biometric Statistics</h2>
           ...
           <div className="flex items-center gap-2 z-10">
              <button onClick={() => downloadCSV(tempBatches, `${lga}_Personnel_Distribution`)} className="p-2 bg-white/10 hover:bg-white/20 rounded border border-white/20"><SpreadsheetIcon /></button>
              <button onClick={() => setIsLedgerOpen(true)} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 rounded text-[9px] font-black uppercase shadow-lg transition-all">Audit Logs</button>
           </div>
        </div>
          ...
          {entries.map((e: CIMClearance) => (
            <div key={e.id} className="bg-white p-3 rounded border border-slate-200 flex flex-col hover:bg-slate-50 transition-all group shadow-sm animate-official">
               <div className="flex justify-between items-center mb-2">
                 <div>
                   <h4 className="text-[12px] font-black uppercase text-slate-800 leading-none">{e.month} AUDIT</h4>
                   <p className="text-[7px] font-bold text-slate-400 uppercase mt-1">Submitted: {new Date(e.dateAdded).toLocaleDateString()}</p>
                 </div>
                 <div className="flex gap-4 items-center">
                   ...
                   <div className="flex gap-1 no-print opacity-0 group-hover:opacity-100 transition-opacity">
                     <button onClick={() => shareData(`Audit Report ${e.month}`, `Cleared: ${e.clearedCount}`)} className="w-6 h-6 flex items-center justify-center text-blue-600 bg-blue-50 rounded border scale-75 hover:shadow-md"><ShareIcon /></button>
                     <button onClick={() => generateOfficialPDF(e, 'CIM_AUDIT')} className="w-6 h-6 flex items-center justify-center text-emerald-600 bg-emerald-50 rounded border scale-75 hover:shadow-md"><DownloadIcon /></button>
               ...
               {e.ziMinute && (
                 <div className="p-2 bg-emerald-50 rounded border border-emerald-100 mt-1 relative">
                   <p className="text-[7px] font-black text-emerald-800 uppercase tracking-widest mb-1">ZI Headquarters Directive:</p>
                   <p className="text-[10px] text-slate-600 leading-relaxed italic">"{e.ziMinute}"</p>
                 </div>
               )}
            </div>
          ))}
          {entries.length === 0 && <p className="text-center text-slate-300 font-bold uppercase text-[9px] py-10">No monthly audits logged yet.</p>}
        </div>
      </div>
      
      {isLedgerOpen && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-[2000] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-3xl rounded-lg shadow-2xl flex flex-col h-[75vh] animate-official">
            <div className="bg-[#004d40] p-4 text-white flex justify-between items-center shrink-0">
               <h3 className="text-[14px] font-black uppercase tracking-tight">Station Exception Registry ({lga})</h3>
               <button onClick={() => setIsLedgerOpen(false)} className="text-xl hover:text-red-200 transition-colors">✕</button>
            </div>
            <div className="flex-1 overflow-auto p-4 custom-scrollbar">
               <table className="w-full">
                  <thead className="text-[8px] font-bold uppercase text-slate-400 text-left border-b"><tr className="pb-1"><th>Corps Member</th><th>Station Context</th><th className="text-right">Administrative Action</th></tr></thead>
                  <tbody className="divide-y divide-slate-50">
                    {entries.reduce((acc: any[], entry: any) => [...acc, ...(entry.unclearedList || []).map((cm: any) => ({ ...cm, lga: entry.lga, month: entry.month }))], []).map((cm: any, idx: number) => (
                        <tr key={idx} className="hover:bg-slate-50 transition-all">
                          ...
                          <td className="py-2 text-right">
                             <div className="flex items-center justify-end gap-1.5">
                               <button onClick={() => handleIssueQuery(cm)} disabled={isGenerating} className="px-3 py-1 bg-[#004d40] text-white text-[8px] font-bold uppercase rounded hover:bg-black disabled:opacity-50 transition-all">Process Query</button>
                               <button onClick={() => (window as any).open(`https://wa.me/?text=${encodeURIComponent(`Administrative Notice: ${cm.name} (${cm.code}) defaulted in biometric clearance for ${cm.month}. Please report to the LGA office immediately.`)}`)} className="w-6 h-6 flex items-center justify-center text-emerald-600 bg-white border rounded hover:shadow-md"><WhatsAppIcon /></button>
                             </div>
                          </td>
                        </tr>
                      ))}
...
/* --- CD&R Module (Minute Sheet Interface) --- */
const CDRModule = ({ entries, lga, db, userRole }: any) => {
  ...
  const handleFileUpload = async (id: string, files: FileList | null) => {
    ...
    window.alert("Document linked to case docket.");
  };
    ...
    <>
      <div className="w-full lg:w-[280px] shrink-0 no-print">
        <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200 lg:sticky lg:top-14">
          ...
          <form onSubmit={handleSubmit} className="space-y-2">
            ...
            <input required placeholder="UNIT / PPA" className="w-full p-2 bg-slate-50 border rounded text-[10px] uppercase outline-none font-bold" value={formData.ppa} onChange={e => setFormData({...formData, ppa: e.target.value.toUpperCase()})} />
            <textarea required placeholder="MISCONDUCT NARRATIVE..." className="w-full p-2 bg-slate-50 border rounded h-24 text-[10px] outline-none" value={formData.misconduct} onChange={e => setFormData({...formData, misconduct: e.target.value})} />
      ...
      <div className="flex-1 space-y-4">
        <div className="flex justify-between items-center mb-1 px-1">
          ...
          <button onClick={() => downloadCSV(entries, "Disciplinary_Case_Index")} className="text-[8px] font-black uppercase text-emerald-700 hover:underline flex items-center gap-1"><SpreadsheetIcon /> Export Registry</button>
        </div>
        {entries.map((cm: CDRCase) => (
          <div key={cm.id} className="bg-white p-5 rounded-lg border border-slate-200 shadow-sm relative animate-official group">
             <div className="absolute top-4 right-4 flex items-center gap-2 no-print">
                ...
                {userRole === 'ZI' && <button onClick={() => deleteData(db, "cdr_cases", cm.id)} className="text-slate-200 hover:text-red-500 scale-75 transition-colors"><TrashIcon /></button>}
             </div>
             <div className="mb-3">
               <h4 className="text-[14px] font-black uppercase tracking-tight text-slate-800 leading-none">{cm.name}</h4>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-slate-50 pt-3 mb-3">
                <div className="space-y-2">
                   <div className="flex justify-between items-center">
                      <p className="text-[8px] font-black text-blue-800 uppercase tracking-widest">Minute: LGI Administrative Remark</p>
                      {cm.responseImage && <button onClick={() => setPreviewImage(cm.responseImage!)} className="text-[7px] font-bold text-blue-600 underline">View Statement</button>}
                   </div>
                   <textarea 
                     readOnly={userRole !== 'LGI'}
                     className={`w-full p-2.5 rounded border text-[10px] h-20 outline-none italic leading-normal ${userRole === 'LGI' ? 'bg-blue-50/20 border-blue-100 focus:bg-white' : 'bg-slate-50 border-slate-100 text-slate-500'}`}
                     placeholder="Enter LGI minute..."
                   ...
                   {userRole === 'LGI' && (
                     <div className="flex gap-2">
                        ...
                        <button onClick={() => handleStatusUpdate(cm.id, 'Forwarded_to_ZI')} className="px-3 py-1 bg-blue-600 text-white rounded text-[8px] font-bold uppercase shadow-sm">Transmit</button>
                     </div>
                   )}
                </div>
                <div className="space-y-2">
                   <p className="text-[8px] font-black text-emerald-800 uppercase tracking-widest">Minute: ZI Headquarters Directive</p>
                   <textarea 
                     readOnly={userRole !== 'ZI'}
                     className={`w-full p-2.5 rounded border text-[10px] h-20 outline-none italic leading-normal ${userRole === 'ZI' ? 'bg-emerald-50/20 border-emerald-100 focus:bg-white' : 'bg-slate-50 border-slate-100 text-slate-500'}`}
                   ...
                   {userRole === 'ZI' && (
                     <div className="flex flex-col gap-1">
                        <div className="grid grid-cols-2 gap-1">
                           <button onClick={() => handleStatusUpdate(cm.id, 'Minuted_back_to_LGI')} className="py-1 bg-yellow-600 text-white rounded text-[8px] font-bold uppercase">Return to LGI</button>
                           <button onClick={() => handleStatusUpdate(cm.id, 'Forwarded_to_CDR')} className="py-1 bg-slate-900 text-white rounded text-[8px] font-bold uppercase">Finalize</button>
                        </div>
                        <button onClick={() => handleStatusUpdate(cm.id, 'Minuted_to_CIM')} className="w-full py-1.5 bg-emerald-700 text-white rounded text-[8px] font-black uppercase tracking-widest">Refer to CIM Desk</button>
                     </div>
                   )}
                </div>
             </div>
             ...
             <div className="flex justify-between items-center border-t border-slate-50 pt-3 no-print">
               <p className="text-[7px] font-black text-slate-300 uppercase tracking-widest">REF: {cm.id.substring(0,8).toUpperCase()}</p>
               <div className="flex gap-2">
                 <button onClick={() => shareData(`Case: ${cm.name}`, cm.misconduct)} className="w-8 h-8 flex items-center justify-center text-blue-600 bg-white border rounded scale-75 shadow-sm hover:shadow-md transition-all"><ShareIcon /></button>
                 <button onClick={() => generateOfficialPDF(cm, 'CDR_CASE')} className="w-8 h-8 flex items-center justify-center text-slate-400 bg-white border rounded scale-75 shadow-sm hover:shadow-md transition-all"><DownloadIcon /></button>
                 <button className="w-8 h-8 flex items-center justify-center text-emerald-600 bg-white border rounded scale-75 shadow-sm hover:shadow-md transition-all" onClick={() => (window as any).open(`https://wa.me/?text=${encodeURIComponent(`Administrative Record Updated for ${cm.name} (${cm.stateCode}).`)}`)}><WhatsAppIcon /></button>
               </div>
             </div>
          </div>
        ))}
        {entries.length === 0 && <div className="text-center py-16 bg-white rounded-lg border border-dashed border-slate-200"><p className="text-slate-300 uppercase font-black tracking-widest text-[9px]">Case Registry Vacant</p></div>}
      </div>
      ...
      {previewImage && (
        <div className="fixed inset-0 bg-slate-950/95 z-[3000] flex items-center justify-center p-4" onClick={() => setPreviewImage(null)}>
          <div className="max-w-4xl w-full max-h-[90vh] flex flex-col items-center gap-4 animate-official">
             ...
             <button className="px-6 py-2 bg-white text-black font-black uppercase text-[10px] rounded shadow-lg hover:bg-slate-200 transition-colors">Close Exhibit</button>
          </div>
        </div>
      )}
    </>
  ...
  return (
    <>
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200">
          ...
          <form onSubmit={async (e) => { e.preventDefault(); await addData(db, "saed_centers", { ...formData, lga }); setFormData({centerName:'',address:'',cmCount:0,fee:0}); }} className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
               <input type="number" placeholder="CENSUS" className="w-full p-2 bg-white rounded border text-[10px] font-black text-blue-600" value={formData.cmCount || ''} onChange={e => setFormData({...formData, cmCount: parseInt(e.target.value) || 0})} />
      ...
      <div className="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 content-start">
        {entries.map((c: any) => (
          <div key={c.id} className="bg-white p-4 rounded border border-slate-100 relative group animate-official hover:shadow-lg overflow-hidden h-fit transition-all">
            ...
            <div className="flex gap-6 pt-3 border-t border-slate-50 mt-3">
               <div><p className="text-[7px] font-bold text-slate-300 mb-0.5 uppercase tracking-widest">Enrolled</p><p className="text-[16px] font-black text-[#004d40]">{c.cmCount}</p></div>
               <div><p className="text-[7px] font-bold text-slate-300 mb-0.5 uppercase tracking-widest">Fee</p><p className="text-[16px] font-black text-emerald-600">₦{Number(c.fee).toLocaleString()}</p></div>
            </div>
            <div className="absolute top-4 right-4 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity no-print">
              <button onClick={() => shareData(`SAED Hub: ${c.centerName}`, `Census: ${c.cmCount} | Station: ${c.lga}`)} className="text-blue-500 scale-75 hover:scale-90 transition-transform"><ShareIcon /></button>
        ...
        {entries.length === 0 && <div className="col-span-full py-16 text-center text-slate-300 uppercase font-black text-[9px]">No SAED hubs registered.</div>}
      </div>
    </>
  );
};
...
const CWHSModule = ({ entries, db, lga, userRole }: any) => {
  ...
  return (
    <>
      <div className="w-full lg:w-[280px] shrink-0 no-print">
        <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200 lg:sticky lg:top-14">
          <h3 className="font-bold uppercase text-[8px] text-slate-400 tracking-widest mb-3">Status Filing Terminal</h3>
          <form onSubmit={async (e) => { e.preventDefault(); await addData(db, "nysc_reports", { ...formData, lga }); setFormData({name:'',stateCode:'',category:ReportCategory.ABSCONDED,details:''}); window.alert("Filed."); }} className="space-y-2">
            <input required placeholder="PERSONNEL NAME" className="w-full p-2 bg-slate-50 rounded border text-[10px] font-black uppercase" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value.toUpperCase()})} />
            ...
            <textarea placeholder="INCIDENT BRIEF..." className="w-full p-2 bg-slate-50 rounded border h-24 text-[10px] outline-none" value={formData.details} onChange={e => setFormData({...formData, details: e.target.value})} />
      ...
      <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3 content-start">
        {entries.map((e: any) => (
          <div key={e.id} className="bg-white p-4 rounded border border-slate-200 relative group animate-official shadow-sm overflow-hidden h-fit transition-all hover:shadow-md">
            ...
            <div className="p-3 bg-slate-50 rounded border border-slate-100 my-3 shadow-inner"><p className="text-[10px] text-slate-600 italic leading-relaxed">"{e.details || 'Documentation pending.'}"</p></div>
            <div className="flex justify-between items-center border-t border-slate-50 pt-3">
               ...
               <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity no-print">
                 <button onClick={() => shareData(`Incident: ${e.name}`, e.details)} className="text-blue-500 scale-75 hover:scale-90 transition-all"><ShareIcon /></button>
        ...
        {entries.length === 0 && <div className="col-span-full py-16 text-center text-slate-300 uppercase font-black text-[9px]">No status reports indexed.</div>}
      </div>
    </>
  );
};
      ...
      <div className="w-full lg:w-[280px] flex flex-col gap-4 no-print shrink-0">
        <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200">
          <div className="flex bg-slate-50 p-1 rounded-md mb-4 border border-slate-100">
             <button onClick={() => setView('UNITS')} className={`flex-1 py-1.5 rounded text-[8px] font-black uppercase transition-all ${view === 'UNITS' ? 'bg-[#004d40] text-white shadow-sm' : 'text-slate-400'}`}>CDS Units</button>
          ...
          {view === 'UNITS' ? (
            <div className="animate-official">
              ...
              <form onSubmit={async (e) => { e.preventDefault(); await addData(db, "cds_groups", { ...groupForm, lga }); setGroupForm({groupName:'',meetingDay:'Wednesday'}); window.alert("Unit Logged."); }} className="space-y-2">
            ...
            <div className="animate-official">
              <h3 className="font-bold uppercase text-[7px] text-slate-400 tracking-widest mb-3">Register Personal project</h3>
              <form onSubmit={async (e) => { e.preventDefault(); await addData(db, "cds_projects", { ...projectForm, lga }); setProjectForm({cmName:'',stateCode:'',projectName:'',description:'',status:'Ongoing'}); window.alert("Project Logged."); }} className="space-y-2">
                <input required placeholder="CORPS MEMBER" className="w-full p-2 bg-slate-50 rounded border text-[10px] font-bold uppercase outline-none" value={projectForm.cmName} onChange={e => setProjectForm({...projectForm, cmName: e.target.value.toUpperCase()})} />
      ...
      <div className="flex-1">
        {view === 'UNITS' ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 content-start animate-official">
            {groups.map((g: any) => (
              <div key={g.id} className="bg-white p-3 rounded border border-slate-200 relative group shadow-sm overflow-hidden h-fit transition-all hover:shadow-md">
                ...
                <div className="absolute top-3 right-3 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity no-print">
                  <button onClick={() => shareData(`CDS Unit: ${g.groupName}`, `Meeting Day: ${g.meetingDay}`)} className="text-blue-500 scale-75 hover:scale-90 transition-all"><ShareIcon /></button>
            ...
            {groups.length === 0 && <p className="col-span-full text-center text-slate-400 py-10 uppercase font-black text-[9px]">No CDS units indexed.</p>}
          </div>
        ) : (
              ...
              <div key={p.id} className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm relative group overflow-hidden h-fit transition-all hover:shadow-md">
                ...
                <div className="flex justify-between items-start mb-2">
                   <div>
                      ...
                      <p className="text-[9px] font-bold text-slate-400 uppercase mt-1">Project Lead: {p.cmName} ({p.stateCode})</p>
                   </div>
                   <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity no-print">
                      <button onClick={() => shareData(`Project Impact: ${p.projectName}`, `${p.cmName}: ${p.description}`)} className="p-1.5 text-blue-500 hover:bg-blue-50 rounded transition-all"><ShareIcon /></button>
                ...
                <div className="flex justify-between items-center text-[7px] font-black uppercase tracking-widest text-slate-400">
                   <span className="bg-slate-100 px-1.5 py-0.5 rounded border border-slate-200">{p.lga} STATION</span>
                   <span>FILED: {new Date(p.dateAdded).toLocaleDateString()}</span>
                </div>
              </div>
            ))}
            {projects.length === 0 && <p className="col-span-full text-center text-slate-400 py-10 uppercase font-black text-[9px]">No individual projects logged.</p>}
          </div>
        )}
      </div>
    </>
